...
import { allUnits, loadUnit, loadCourse, standalone } from '../../tools/_courses.mjs';
...
function runLearner(seed, policy = 'engine', opts = {}) {
  ...
  for (let step = 0; step < budget; step++) {
    ...
    if (!frozen && !process.env.DIAG_NODECAY) for (const s of SKILLS) k.set(s, k.get(s) * (1 - DECAY / stability.get(s)));
...
console.log('\n  PER-SKILL final hidden competence (mean, and share >= 0.85)');
for (const sk of SKILLS) {
  const ks = results.map((r) => r.k.get(sk));
  const mu = ks.reduce((x, y) => x + y, 0) / ks.length;
  const ok = ks.filter((x) => x >= 0.85).length / ks.length;
  const mastered = results.filter((r) => r.engineMasteredSet.has(sk)).length / results.length;
  console.log(`    ${sk.padEnd(24)} k=${mu.toFixed(3)}  >=0.85 ${(100*ok).toFixed(0)}%  engine-mastered ${(100*mastered).toFixed(0)}%`);
}
