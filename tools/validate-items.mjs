...
// ---------------------------------------------------------------------------
// 11. The Level 1 crosswalk — one row per skill, both frameworks side by side.
//
...
//
// IT USED TO BE WRITTEN TO content/STANDARDS.md, WHICH IS NOT WHAT IT IS.
// This file loads one unit, so the document it produced was titled "Standards
// alignment — ASCENT, Algebra I Level 1", covered ten skills and listed
// seventeen TEKS expectations — while five units ship, carrying thirty-eight
// Algebra I student expectations between them. It was the top document of a
// product called Algebra I and it was about a fifth of it. So it is written
// under its real name, beside the four generated unit maps, and
// content/STANDARDS.md is now built by tools/build-standards.mjs from every
// unit in the manifest. Both are still byte-compared; neither can drift.
// ---------------------------------------------------------------------------
const CROSSWALK_PATH = path.join(ROOT, 'content/standards/algebra1-l1-crosswalk.md');
function crosswalk() {
  const short = (c) => c.replace('CCSS.MATH.CONTENT.', '');
  const REPNAME = {
    symbolic: 'symbolic', table: 'table', graph: 'graph', verbal: 'verbal', context: 'contextual', figure: 'figure',
...
p('| | |');
p('|---|---|');
p(`| **Common Core** | ${ccss.standards.length} content standards, ${ccss.practices.length} practice standards. Working paper: [\`algebra1-l1-standards-map.md\`](algebra1-l1-standards-map.md) |`);
p(`| **TEKS** | ${teks.standards.length} student expectations, ${teks.processStandards.length} process standards, ${teks.gaps.length} declared gaps. Source data: [\`teks-algebra1-l1.json\`](teks-algebra1-l1.json) |`);
...
p('| **Scope** | Level 1 only. The course-level document, covering every unit, is [`content/STANDARDS.md`](../STANDARDS.md). |');
...
}
{
  ...
  if (process.argv.includes('--write-map')) {
    ...
    if (have == null) fail('teks: the Level 1 crosswalk is missing (run with --write-map)');
    else if (have !== want) fail('teks: the Level 1 crosswalk is out of step with the data it is generated from (run with --write-map)');
  }
}
...
if (problems.length) {
  ...
  console.error(`\n${problems.length} problems (showing ${shown.length}):`);
  shown.forEach((p) => console.error('  ' + p));
} else {
  console.log('every item independently re-derived, strict-KaTeX clean, misconception-tagged, and standards-aligned in all three languages');
}
/* THE LEDGER OWNS THE EXIT CODE — tools/_findings.mjs. This gate reads the
   SHIPPED Level 1 bank, which is the first thing a learner is handed. */
findings('check:items', { scope: 'route' }).route(problems.map(String)).done();
